...
class SchoolMbtisController extends Controller
{
    public function showDownloadList(Request $request, School $school)
    {
        if ($school->mbti_epps_lss_count <= 0) {
            return redirect()->back()->with('error', 'Tidak ada data MBTI');
        }

        $downloadable = collect(range(1, $school->mbti_epps_lss_count));
        $downloadableCount = 25;

        return view('school-mbtis.download-list', compact('school', 'downloadable', 'downloadableCount'));
    }
    ...
    public function download(Request $request, School $school)
    {
        ...
        if ($request->has('offset')) {
            ...
            $fileName = "Laporan MBTI {$school->name} Nomor Urut {$start} - {$end}";
